<?php require 'views/header.php'; ?>
 ...
 echo '<li><a href="'.URL.'/urunler/detay/'.$value["id"].'/'. $Harici->seo($value["urunad"]).'">'.$value["urunad"].'</a></li>';
	...
	<span> <?php echo number_format($veri["data1"][0]["fiyat"],2,'.',',');
					...
					<?php
					// KUMAŞ@Keten,ÜRETİM YERİ@Japonya,RENK@Sarı
					$gelenozellikler=explode(",",$veri["data1"][0]["ozellikler"]);					
				
					
					foreach ($gelenozellikler as $value):
					 // ÜRETİM YERİ@Japonya
					$parcalandi=explode("@",$value);	
					// 	$parcalandi[0]	özelliğim adı
					// $parcalandi[1]	özelliğimin verisi
					
					echo '<div class="span span1">
						<p class="left">'.$parcalandi[0].'</p>
						<p class="right">'.$parcalandi[1].'</p>
						<div class="clearfix"></div>
					</div>';
					...
					<div class="purchase">
                        ...
                        <div class="col-md-6" id="sonuc"></div>
               ...
               <input type="number" min="1" max="10" value="1" name="adet" class="form-control" />  
                      ...
                      <div class="col-md-3">
             <input type="hidden" name="id" value="<?php echo $veri["data1"][0]["id"]; ?>"  />  
                      <input type="button" class="btn hizala_3" value="Sepete Ekle" id="SepetBtn"></div>   
                       </form>
						...
						<div class="row mt-2" id="FavoriMevcut"> 
                       
                        <div class="col-md-9" id="sonuc"></div>                              
                       
                        
                      <div class="col-md-3">
					   <form id="FavoriForm">
             		<input type="hidden" name="urunlink" value="<?php echo $_SERVER['SERVER_NAME'].$_SERVER["REQUEST_URI"]; ?>"  />  
					<input type="hidden" name="uyeid" value="<?php echo Session::get("uye"); ?>"  />  
					
				<input type="hidden" name="urunresim" value="<?php echo URL; ?>/views/design/images/<?php  echo $veri["data1"][0]["res1"]; ?>" />  
					
                     <input type="button" class="btn btn-success mt-2" value="Favoriye Ekle" id="FavoriBtn">
					  </form>
					 </div>                     
            ...
            <div class="row" id="FormSonuc"></div>
             ...
             <div class="row arkaplan" id="FormAnasi">
                ...
                <input type="text" name="ad" class="form-control" value="<?php echo Session::get("kulad"); ?>" readonly="readonly"/>
                </div>
...
<img src="'.URL.'/views/design/images/'.$value["res1"].'" alt="'.$value["urunad"].'" /></a>
						...
						<p><a class="item_add" href="'.URL.'/urunler/detay/'.$value["id"].'/'. $Harici->seo($value["urunad"]).'"><i></i> <span class="item_price">'. number_format($value["fiyat"],2,'.',',').'</span></a></p>
					</div>';
